...
{
	if(IIC2_ReadBytes(Color_Adress<<1,RGB_Reg,Result,len))return 1;
	else return 0;
}
unsigned char IIC_Get_HSL(unsigned char * Result,unsigned char len)
{
	if(IIC2_ReadBytes(Color_Adress<<1,HSL_Reg,Result,len))return 1;
	else return 0;
}
...
{
	if(IIC3_ReadBytes(GW_GRAY_ADDR_DEF<<1,GW_GRAY_ANALOG_BASE_,Result,len))return 1;
	else return 0;
}

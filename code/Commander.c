...
void motion_StateManager_Init(void)
{
    ...
    fsm_Add_Transition(Back_beginC3, Stop_mode, ConditionC6);
    fsm_Add_Transition(Back_beginC1, Stop_mode, ConditionC6);
    fsm_Add_Transition(Back_beginC2, Stop_mode, ConditionC6);
    fsm_Add_Transition(Back_beginC3, Stop_mode, ConditionC6);
...
void Back_beginA1_StateFunc(void *data)
{
    ...
    if(delivery_count==2){
      ...
      while (cross_flag!=1)
        {
        LineTracking();
     if(gray_front[0]==0&&gray_front[7]==0)
    {
        cross_ready=1;
    }
...
void Back_endA1_StateFunc(void *data)
{
    ...
    {
    case 0:
        Move_Right_Position(0.32f);
        vTaskDelay(1500);
        Move_Forward_Position(0.410f);
        vTaskDelay(1500);
        Yuntai_set_Angle(98);
        vTaskDelay(3200);
        Solenoid_Close();
...
void Delivery_beginA2_StateFunc(void *data)
{
    ...
    {
    LineTracking();
    if(( gray_front[7] == 0)||(gray_front[0]== 0))
    {
        cross_ready=1;
    }
    if (gray_right[3]==0&&cross_ready==1)
...
void Delivery_beginA3_StateFunc(void *data)
{
    ...
    {
    LineTracking();
    if((gray_front[5]==0&&gray_front[6]== 0 && gray_front[7] == 0)||(gray_front[0]== 0 && gray_front[1] == 0&&gray_front[2]==0))
    {
        cross_ready=1;
    }
    if (gray_right[3]==0&&cross_ready==1)
...
void Delivery_beginB1_StateFunc(void *data){
    ...
    {
    LineTracking();
    if(gray_front[1]==0&&gray_front[7]==0)
    {
        cross_ready=1;
    }
    if (gray_right[3]==0&&cross_ready==1)
...
void Back_endC1_StateFunc(void *data){
    ...
    {
    case 0:
        Move_Left_Position(0.305f);
        vTaskDelay(1700);
        Move_Forward_Position(0.405f);
        vTaskDelay(1700);
        Yuntai_set_Angle(105);
        vTaskDelay(1500);
        Solenoid_Close();
        Pump_Open();
        Sigancatch();
        ...
        move_to_zero();
        vTaskDelay(1500);
        Move_Backward_Position(0.405f);
        vTaskDelay(2200);
        Move_Right_Position(0.305f);
        vTaskDelay(500);
        Yuntai_set_Angle(65);
        vTaskDelay(1000);
        Motorangle(180.0f);
